...
shaka.util.PeriodCombiner = class {
  ...
  /** @override */
  release() {
    const allStreams = this.audioStreams_.concat(this.videoStreams_, this.textStreams_);
    ...
    if (!this.videoStreams_.length || !this.audioStreams_.length) {
      ...
      for (const audio of this.audioStreams_) {
        for (const video of this.videoStreams_) {
          const commonDrmInfos = shaka.media.DrmEngine.getCommonDrmInfos(audio.drmInfos, video.drmInfos);

          if (audio.drmInfos.length && video.drmInfos.length && !commonDrmInfos.length) {
            shaka.log.warning('Incompatible DRM in audio & video, skipping variant creation.', audio, video);
            continue;
          }

          if (audio.height === video.height && audio.frameRate === video.frameRate) {
            const id = nextVariantId++;
            variants.push({
              id,
  ...
  static async combineDbStreams(streamDbsPerPeriod) {
    ...
    return combinedVideoStreamDbs.concat(combinedAudioStreamDbs).concat(combinedTextStreamDbs);
  }
      ...
      for (const stream of unusedStreams) {
        ...
        const hasCodec = outputStreams.some((s) => {
          return (
            ...
            shaka.util.MimeUtils.getCodecBase(s.codecs) == shaka.util.MimeUtils.getCodecBase(stream.codecs)
          );
        });
        if (!hasCodec) {
          continue;
  ...
  static async extendExistingOutputStream_(
    ...
    const matches = shaka.util.PeriodCombiner.findMatchesInAllPeriods_(streamsPerPeriod, outputStream);
   ...
   * @private
   */
  static async createNewOutputStream_(stream, streamsPerPeriod, clone, concat, unusedStreamsPerPeriod) {
   ...
   * @private
   */
  static extendOutputStream_(outputStream, matches, firstNewPeriodIndex, concat, unusedStreamsPerPeriod) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
  ...
  static concatenateStreams_(output, input) {
    ...
    const commonDrmInfos = shaka.media.DrmEngine.getCommonDrmInfos(output.drmInfos, input.drmInfos);
    ...
    goog.asserts.assert(input.segmentIndex, 'Input segment index should have been created by now!');
    ...
    if (input.trickModeVideo) {
      if (!output.trickModeVideo) {
        ...
        output.trickModeVideo = shaka.util.PeriodCombiner.cloneStream_(input.trickModeVideo);
      ...
      shaka.util.PeriodCombiner.concatenateStreams_(output.trickModeVideo, input);
    }
  }
  ...
  static findMatchesInAllPeriods_(streamsPerPeriod, outputStream) {
    ...
    for (const streams of streamsPerPeriod) {
      const match = shaka.util.PeriodCombiner.findBestMatchInPeriod_(streams, outputStream);
      if (!match) {
        return null;
      }
      matches.push(match);
  ...
  static areAVStreamsCompatible_(outputStream, candidate) {
    ...
    if (outputStream.drmInfos) {
      ...
      if (!shaka.media.DrmEngine.areDrmCompatible(outputStream.drmInfos, candidate.drmInfos)) {
        return false;
      }
    }
  ...
  static isAudioStreamBetterMatch_(outputStream, best, candidate) {
    ...
    const channelsBetterOrWorse = shaka.util.PeriodCombiner.compareClosestPreferLower(
    ...
    const sampleRateBetterOrWorse = shaka.util.PeriodCombiner.compareClosestPreferLower(
    ...
    if (outputStream.bandwidth) {
      ...
      const bandwidthBetterOrWorse = shaka.util.PeriodCombiner.compareClosestPreferMinimalAbsDiff_(
  ...
  static isVideoStreamBetterMatch_(outputStream, best, candidate) {
    ...
    const resolutionBetterOrWorse = shaka.util.PeriodCombiner.compareClosestPreferLower(
    ...
    if (outputStream.frameRate) {
      ...
      const frameRateBetterOrWorse = shaka.util.PeriodCombiner.compareClosestPreferLower(
