...
shaka.hls.HlsParser = class {
    ...
    for (const audioInfo of audioInfos) {
      for (const videoInfo of videoInfos) {
        ...
        if (audioStream && videoStream) {
          ...
          // CUSTOM GAMELEAP CODE
          if (audioStream.height !== videoStream.height) {
            // This happens when audio and video aren't intended for each other </3
            shaka.log.debug('Skipping variant because audio and video height mismatch.');
            continue;
          }
        }
  ...
  async createStreamInfo_(verbatimMediaPlaylistUri, codecs, type, language,
    ...
    const rootUriFolder = (verbatimMediaPlaylistUri && verbatimMediaPlaylistUri.split('/')[0]) || '';
    ...
    const stream = {
      ...
      width: undefined,
      height: parseInt(rootUriFolder, 10) || undefined,
      bandwidth: undefined,
      roles: roles,
      forced: forced,
      channelsCount,
