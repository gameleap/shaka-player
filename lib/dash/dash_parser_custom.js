...
shaka.dash.DashParser = class {
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    /** @type {!Array.<string>} */
    const locations = XmlUtils.findChildren(mpd, 'Location').map(XmlUtils.getContents).filter(Functional.isNotNull);
    if (locations.length > 0) {
      const absoluteLocations = shaka.util.ManifestParserUtils.resolveUris(manifestBaseUris, locations);
    ...
    const baseUris = shaka.util.ManifestParserUtils.resolveUris(manifestBaseUris, uris);
    ...
    const maxSegmentDuration = XmlUtils.parseAttr(mpd, 'maxSegmentDuration', XmlUtils.parseDuration);
  ...
  parsePeriods_(context, baseUris, mpd) {
    ...
    for (const { i, item: elem, next } of enumerate(periodNodes)) {
      const start = /** @type {number} */ (XmlUtils.parseAttr(elem, 'start', XmlUtils.parseDuration, prevEnd));
      ...
      const threshold = shaka.util.ManifestParserUtils.GAP_OVERLAP_TOLERANCE_SECONDS;
    ...
    if (presentationDuration != null) {
      if (prevEnd != presentationDuration) {
        shaka.log.warning('@mediaPresentationDuration does not match the total duration of ', 'all Periods.');
  ...
  parsePeriod_(context, baseUris, periodInfo) {
    ...
    if (!context.period.id) {
      shaka.log.info('No Period ID given for Period with start time ' + periodInfo.start + ',  Assigning a default');
        ...
        if (targetIds.includes(normalSet.id)) {
          for (const stream of normalSet.streams) {
            ...
            stream.trickModeVideo = trickModeSet.streams.find(
              (trickStream) => MimeUtils.getCodecBase(stream.codecs) == MimeUtils.getCodecBase(trickStream.codecs)
            );
          }
        }
      }
  ...
  parseAdaptationSet_(context, elem) {
    ...
    const language = shaka.util.LanguageUtils.normalize(elem.getAttribute('lang') || 'und');
    ...
    const streams = representations
      .map((representation) => {
        return this.parseRepresentation_(
          context,
          contentProtection,
          kind,
          language,
    ...
    if (!context.adaptationSet.contentType || context.adaptationSet.contentType == ContentType.APPLICATION) {
      ...
      context.adaptationSet.contentType = shaka.dash.DashParser.guessContentType_(mimeType, codecs);
  ...
  parseRepresentation_(context, contentProtection, kind, language, label, isPrimary, roles, closedCaptions, node) {
    ...
    try {
      ...
      if (context.representation.segmentBase) {
        streamInfo = shaka.dash.SegmentBase.createStreamInfo(context, requestInitSegment);
      } else if (context.representation.segmentList) {
        streamInfo = shaka.dash.SegmentList.createStreamInfo(context, this.segmentIndexMap_);
        ...
        streamInfo = {
          generateSegmentIndex: () => {
            return Promise.resolve(shaka.media.SegmentIndex.forSingleSegment(periodStart, duration, baseUris));
          },
        };
      }
    } catch (error) {
    ...
    const stream = {
      ...
      closedCaptions,
      originalAdaptationName: context.representation.rootUriFolder,
    };
  ...
  async onUpdate_() {
    ...
    try {
      ...
      goog.asserts.assert(error instanceof shaka.util.Error, 'Should only receive a Shaka error');
  ...
  createFrame_(elem, parent, baseUris) {
    ...
    const uris = uriObjs.map(XmlUtils.getContents);
    const rootUriFolder = (uris[0] && uris[0].split('/')[0]) || '';
    const rootUriFolderRes = rootUriFolder ? parseInt(rootUriFolder.split('_')[0], 10) : undefined;
    const rootUriFolderFramerate = rootUriFolder ? parseInt(rootUriFolder.split('_')[1], 10) : undefined;
    ...
    const frameRate = rootUriFolderFramerate || XmlUtils.parseAttr(elem, 'frameRate', evalDivision) || parent.frameRate;
    ...
    const segmentBaseAto = segmentBase
      ? XmlUtils.parseAttr(segmentBase, 'availabilityTimeOffset', XmlUtils.parseFloat) || 0
      : 0;
    const segmentTemplateAto = segmentTemplate
      ? XmlUtils.parseAttr(segmentTemplate, 'availabilityTimeOffset', XmlUtils.parseFloat) || 0
      : 0;
    const baseUriAto =
      uriObjs && uriObjs.length
        ? XmlUtils.parseAttr(uriObjs[0], 'availabilityTimeOffset', XmlUtils.parseFloat) || 0
        : 0;
    ...
    return {
      ...
      height: XmlUtils.parseAttr(elem, 'height', parseNumber) || parent.height || rootUriFolderRes,
    ...
    for (const elem of audioChannelConfigs) {
      ...
      switch (scheme) {
        ...
        case 'urn:dts:dash:audio_channel_configuration:2012': {
          ...
          if (!intValue) {
            // 0 or NaN
            shaka.log.warning('Channel parsing failure! ' + 'Ignoring scheme and value', scheme, value);
            continue;
          }
          return intValue;
        }
  ...
  async requestForTiming_(baseUris, uri, method) {
    const requestUris = shaka.util.ManifestParserUtils.resolveUris(baseUris, [uri]);
    ...
    if (method == 'HEAD') {
      if (!response.headers || !response.headers['date']) {
        shaka.log.warning('UTC timing response is missing', 'expected date header');
        return 0;
      }
      ...
      try {
        ...
        switch (scheme) {
          ...
          case 'urn:mpeg:dash:utc:sntp:2014':
            ...
            break;
          default:
            shaka.log.alwaysWarn('Unrecognized scheme in UTCTiming element', scheme);
            break;
        }
      } catch (e) {
 ...
 * @property {?string} rootUriFolder
 *   The root folder of the base URI.
 */
...
shaka.media.ManifestParser.registerParserByExtension('mpd', () => new shaka.dash.DashParser());
shaka.media.ManifestParser.registerParserByMime('application/dash+xml', () => new shaka.dash.DashParser());
shaka.media.ManifestParser.registerParserByMime('video/vnd.mpeg.dash.mpd', () => new shaka.dash.DashParser());
